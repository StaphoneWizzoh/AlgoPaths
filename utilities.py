...
def screen_coords_from_grid_pos(pos, grid_dimensions):
    ...
    # 20 is a size of 18 pixels plus a border each side
...
def grid_pos_from_screen_coords(pos, dimensions):
    # Converts screen-based coordinates to grid_based coordinates
    x, y = pos
    m, n = dimensions
    j = int((20 * (n / 2) + x) / 20)
    i = int((20 * (m / 2) - y) / 20)
...
def read_maze_from_file(file_name):
    # Reads a maze stored in a text file and returns a 2-d list containing the maze representation
    try:
        with open(file_name) as fh:
            # Convert string maze to 2-d list
            ...
            for row in maze:
                if len(row) != num_cols_top_row:
                    print("The maze is not rectangular. Please use a rectangular maze.")
                    raise SystemExit
